...
def bfgs_e(func, grad, x0, eps_f=0., eps_g=0., args=None, callback=None, options=None):
    ...
    args : dict, optional
        Extra arguments passed to the objective function and its derivatives (func, grad), exactly like args argument of scipy.optimize.minimize()
    ...
    options : dict, optional
        Options passed to the solver. It optionally contains:
            ...
            max_ls_fails : int, optional
                Maximum number of consecutive line search fails. (Default: None)
    ...
    if 'max_ls_fails' not in options.keys():
        options['max_ls_fails'] = None
    ...
    while iter < options['max_iter']:
        ...
        alpha, beta, mu_hat, f_k, g_new, eps_fk, eps_gk, ls_fevals, ls_gevals, armijo_flag, wolfe_flag, split_flag = \
            _line_search_nt_wolfe(func, grad, x_k, p_k, eps_f=eps_f, eps_g=eps_g, args=args, f_k=f_k, g_k=g_k,
        ...
        # refreshing consecutive line search fails counter
        ...
        # check if maximum consecutive line search fail is reached
        elif options['max_ls_fails'] is not None:
            if cons_ls_fails > options['max_ls_fails']:
                flag = 8
                break
    ...
    if options['display'] >= 1:
        ...
        if flag == 0:
            ...
            print('Terminated! Reached maximum number of consecutive line search fail.')
...
def _line_search_nt_wolfe(func, grad, x_k, p_k, eps_f=0., eps_g=0., args=None, f_k=None, g_k=None, alpha_init=1., beta_init=1.,
    ...
    args : dict, optional
        Extra arguments passed to the objective function and its derivatives (func, grad)
    f_k : float, optional
        ...
        if f_new > f_k + suff_dec + relax:
            ...
            if verbose:
                print(
                    ...
                    % (ls_iter, alpha, f_k, f_new, gtp, eps_fk, eps_fp, np.linalg.norm(eps_gk)))
            ...
            if np.abs(gtp_new - gtp) < (1 + c3) * rhs: #Pourquoi pas 2*(1 + c3) * rhs ?

                # print result
                if verbose:
                    print(
                        '  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  '
                        % (ls_iter, alpha, f_k, f_new, gtp, gtp_new, eps_fk, eps_fp, np.linalg.norm(eps_gk), np.linalg.norm(eps_gp)))
    ...
    if not armijo_flag or not wolfe_flag or split_flag:
        ...
        if f_best < np.inf:
            ...
            alpha, f_new, eps_fp, fevals_back, armijo_flag = \
                _backtracking(func, x_k, p_k, g_k, alpha=alpha, eps_f=eps_f, eps_g=eps_g, args=args, f_k=f_k, eps_fk=eps_fk,
        ...
        if not wolfe_flag:
            # Fixing the case where Armijo condition fails at every initial phase iteration, thus eps_gp hasn't been assigned yet
            if 'eps_gp' not in locals():
                eps_gp = None

            beta, g_new, gtp_new, eps_gp, fevals_length, gevals_length, wolfe_flag = \
                _lengthening(grad, x_k, p_k, beta=beta, eps_g=eps_g, args=args, g_k=g_k, eps_gk=eps_gk, eps_gp=eps_gp, gtp=gtp,
...
def _backtracking(func, x_k, p_k, g_k, alpha=1., eps_f=0., eps_g=0., args=None, f_k=None, eps_fk=None, eps_fp=None,
...
def _lengthening(grad, x_k, p_k, beta=1., eps_g=0., args=None, g_k=None, eps_gk=None, eps_gp=None, gtp=None,
    ...
    while ls_iter < max_ls_iter:
        ...
        if verbose:
            print('  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  '
                  % (ls_iter, beta, gtp, gtp_new, np.linalg.norm(eps_g), np.linalg.norm(eps_gp), rhs))

        # check noise control condition
        if gtp_new - gtp < (1 + c3) * rhs: #Pareil qu'un peu plus haut, pourquoi pas 2*(1 + c3) * rhs comme écrit dans le papier ?
